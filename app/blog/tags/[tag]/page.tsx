...
export async function generateMetadata({ params }: { params: Promise<{ tag: string }> }) {
  ...
  return {
    title: `${tag} Tag - Zyvarin Social`,
    description: `Browse Zyvarin Social blog posts tagged with ${tag}.`
  }
}
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-16 pb-10">
        <div className="flex items-center justify-between gap-4 mb-8">
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-slate-500">Tag</p>
            <h1 className="text-4xl font-semibold text-slate-900 mt-2">#{tag}</h1>
            <p className="text-slate-600 mt-2">Stories connected to this tag.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-6">
            {filteredBlogs.length === 0 ? (
              <div className="bg-white border border-dashed border-slate-200 rounded-2xl p-10 text-center shadow-sm">
                <p className="text-lg font-semibold text-slate-800">Nothing under this tag yet.</p>
                <p className="text-slate-500 mt-2">Try another tag or go back to the blog.</p>
                    ...
                    <div className="md:col-span-3 p-8 flex flex-col gap-4">
                      ...
                      {blog.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {blog.tags.slice(0, 4).map((tagItem) => (
                            <span
                              key={tagItem}
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 space-y-4">
              ...
              <form className="relative" action={`/blog/tags/${encodeURIComponent(tag)}`} method="GET">
                <input
                  type="text"
                  name="q"
                  defaultValue={q}
              ...
              <div className="flex flex-col gap-2">
                {categories.map((cat) => (
                  <Link
                    ...
                    className="flex items-center justify-between rounded-2xl px-4 py-3 text-sm transition bg-slate-50 text-slate-700 hover:bg-slate-100"
                  >
                    <span>{cat}</span>
                  </Link>
                ))}
            ...
            <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((tagItem) => {
                  const active = tagItem === tag
                  return (
                    <Link
                      key={tagItem}
                      href={`/blog/tags/${encodeURIComponent(tagItem)}`}
